from .setting import (
    ...
    SupportLang,
    cardPoolType,
    get_color_five_section,
    get_color_four_section,
    get_params_link,
)
from .src.tools.generator import CardConvene
from .src.tools.model import Calculator, ConveneData
...
class Convene:
    ...
    async def calculator(
        self, data: ConveneData, generator: bool = False, art: str = None
    ) -> Calculator:
        ...
        if art is None:
            art = ArtBanner.get(
                str(data.gacha_id), "https://i.ibb.co/1XcZSzR/119161444-p0-master1200.jpg"
            )
    ...
    async def get(
        self, gacha_id: int | str, lang: str = "en", generator: bool = False, art: str = None
    ) -> ConveneData:
        ...
        """
        if lang not in SupportLang:
            raise TypeError(
                f"Argument lang, incorrectly specified, valid values \u200b\u200bare: {', '.join(SupportLang)}"
            )

        if str(gacha_id) not in cardPoolType:
            raise TypeError(
                "Argument gacha_id, incorrectly specified, valid values \u200bfrom 1 to 7 (1 - Featured Resonator | 2 - Featured Weapon | 3 - Standard Resonator | 4 - Standard Weapon | 5 - Beginner Convene | 6 - Beginner Convene Choice | 7 - Other)"
            )

        if art is None:
            art = ArtBanner.get(
                str(gacha_id), "https://i.ibb.co/1XcZSzR/119161444-p0-master1200.jpg"
            )
