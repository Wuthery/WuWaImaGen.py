...
class Record(BaseModel):
    ...
    async def get_icon(self):
        if len(str(self.resourceId)) > 5:
            ...
            return RecordIcon(
                icon=f"https://api.hakush.in/ww/UI/UIResources/Common/Image/IconRoleHead256/{icon}.webp",
                banner=f"https://api.hakush.in/ww/UI/UIResources/Common/Image/IconRolePile/{banner}.webp",
            )
...
class Calculator(BaseModel):
    ...
    gacha_id: int | None = Field(1)
    card: Image.Image | None = Field(None)
